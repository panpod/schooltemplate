...
if(isset($_POST['title']))
{
	...
	if(isset($_FILES['user_file']))
	{
		...
		if($file_type=='image/jpeg' ||  $file_type=='image/png' )
		{
			...
			{
				//Unlink image 
				$fetch_detail_array = mysql_fetch_assoc(mysql_query("Select * from user_blog  where user_id= '".$user_id."' and id='".$_GET['id']."' "));
				...
				$image_name  = $edit_id.'.'.$file_detail['extension'];
				$image_path  = 'images/blog/'.$image_name;
				move_uploaded_file($tmp_name, "$image_path");
				$update_query = mysql_query("Update user_blog set user_image = '".$image_name."'  where id = '".$edit_id."' ");
	...
	if(!empty($title) && !empty($detail))
	{
		...
		$update_query = mysql_query("Update user_blog set title = '".$title."' , detail = '".$detail."',tag = '".$tag."' where id = '".$edit_id."' ");
	}
}
				...
				<div class="row">
					...
					<!-- BEGIN MAIN CONTENT -->
						...
						<div class=" main col-sm-8">
							<h1 class="center">Edit your blog</h1>
							...
							<div class="col-sm-12">
								<form method="post" id="user_login_form" name="user_login_form" class="form-style" novalidate="novalidate" enctype="multipart/form-data" >
									...
									<input type="text" class="form-control" placeholder="Blog detail page" id="title" value="<?php print($fetch_detail_array['title']); ?>" name="title">
									<input type="text" class="form-control" placeholder="Tags" name="tag" id="tag" value="<?php print($fetch_detail_array['tag']); ?>">
									...
									if(!empty($img))
									{
										echo '<br><img src="images/blog/'.$img.'" height="40" width="80"> <a href="delete_user_blog_img.php?id='.$id.'&img='.$img.'">Delete</a><br>';
									}
									?>
									...
									<br>
									<textarea rows="10" cols="80" name="detail" id="detail"><?php print($fetch_detail_array['detail']); ?></textarea>
