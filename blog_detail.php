...
$fetch_user_blog = mysql_query("Select * from user_blog where id = '".mysql_real_escape_string($_GET['id'])."' order by created desc  ");
$tag_query = '';
if(mysql_num_rows($fetch_user_blog)>0)
{

	$tag_query = mysql_fetch_assoc(mysql_query("Select GROUP_CONCAT(distinct tag,',') as user_tag from user_blog   group by user_id "));

}


$fetch_blog_detail = mysql_fetch_assoc((mysql_query("Select user_blog.*,users.f_name,users.l_name from user_blog inner join users on users.id = user_blog.user_id where  user_blog.id = '".mysql_real_escape_string($_GET['id'])."'  ")));
?>
<!DOCTYPE html>
			...
			<div class="container">
				<div class="row">
					...
					<div class="sidebar gray col-sm-3">
						...
						<?php require_once 'blocks/widgets/common_tags.php'; ?>
						
						<?php require_once 'blocks/widgets/latest_blogs.php'; ?>
					...
					<div class="main col-sm-9">
						...
						<div class="blog-bottom-info">
							...
							<div id="post-author"><?php /* ?><a href="edit_blog.php?id=<?php echo mysql_real_escape_string($_GET['id']); ?>"><i class="fa fa-pencil"></i></a> <?php */ ?> By <?php echo $fetch_blog_detail['f_name'].' '.$fetch_blog_detail['l_name']; ?></div>
						</div>
